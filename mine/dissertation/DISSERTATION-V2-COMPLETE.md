...
### Errata and Corrections (Post-Defense Review)

An independent rigorous critique of this dissertation identified several mathematical and statistical issues that have been corrected in this V2 edition:

1. **Definition 7.3 (Temporal Triangle):** Corrected from redundant ordered triple $(\delta_1, \delta_2, \delta_3)$ with $\delta_3 = \delta_1 + \delta_2$ to the simpler ordered pair $(\delta_1, \delta_2)$. The third component carries zero additional information.

2. **Definition 7.5 (Interval Ratio):** The complex-number formulation using $\omega$ has been replaced with log-temporal coordinates $(X, Y) = (\log \delta_1, \log \delta_2)$, which correctly encodes the 2D nature of the classification. The original formulation collapsed to a 1D line segment.

3. **Theorems 7.9, 7.10, 7.11 (TStream Products, Coproducts, Monad):** Proof sketches have been annotated with identified gaps. These results remain plausible conjectures pending full formal proof.

4. **Theorem 7.16 (DepCat Groupoid):** The equivalence claim has been corrected. The original proof conflated "dependency" with "spawn-return." The corrected version requires typed morphisms.

5. **Definition 7.18 / Proposition 7.19 (Absence Monad):** The monad laws are verified informally only. A full Coq formalization remains future work.

6. **Theorem 7.22 (Raft as 2-Point Snap):** Downgraded to Observation 7.22. The categorical correspondence is trivially true but provides no insight into Raft's actual protocol.

7. **Proposition 7.27 (Fiedler Bound):** Direction corrected and additional assumptions (regular graph) noted as required.

8. **Statistical caveats strengthened:** Hurst exponent estimates ($H \approx 0.7$ "creative constant"), temporal connectome correlations, and the entropy–miss-rate linear fit now carry explicit confidence interval warnings and multiple-test correction notes.

9. **Notation disambiguation:** $H$ is used for harmony (Chapter 5–6), Hurst exponent (Chapter 8), and Shannon entropy. Context disambiguates; a notation table has been added.

10. **The universal claim "absence is the signal" has been fully corrected** to: "Deviations from expectation carry information proportional to their rarity." In rooms with miss rate > 50%, hits are more informative than misses.
...
|--------|---------|----------|
| $H(A,B)$ | Jaccard harmony between agents A and B | Chapters 5–6 |
| $H$ | Hurst exponent | Chapter 8 |
| $H(X)$ | Shannon entropy of random variable X | Chapters 4, 8 |
| $H^1(X, F)$ | First cohomology group | Chapters 3, 7–8 |
| $\mathcal{H}$ | Fleet harmony measure | Chapter 3 |
| $\omega$ | $e^{2\pi i/3}$ (cube root of unity) | Chapter 7 |
| $\theta$ | Temporal angle $= \arctan(b/a)$ | Chapters 3, 7 |
| $(X, Y)$ | Log-temporal coordinates | Chapter 7 |
...
| 7.9 Temporal Calculus | |
| 7.10 Fourier-Eisenstein Conjecture | |
| 7.11 Summary of Formal Results | |
| | |
...
**Caveat on 90.8% figure:** 686 of 895 triangles (76.6%) come from fleet_health, which has 100% steady-state. The global "90.8% steady" statistic reflects fleet_health's dominance of the dataset, not a universal property. When fleet_health is excluded, the remaining 209 triangles from 13 rooms show substantially more diversity, with room-level steady-state rates ranging from 25% to 100%. The 90.8% figure should not be presented as a general finding about agent systems.
...
**Definition 7.3** (Temporal Triangle). Given three consecutive present temporal points $p_i = (t_i)$, $p_{i+1} = (t_{i+1})$, $p_{i+2} = (t_{i+2})$ in a pure temporal stream, the *temporal triangle* $\tau_i$ is the ordered pair of intervals:

$$\tau_i = (a, b) = (t_{i+1} - t_i,\; t_{i+2} - t_{i+1}) \in \mathbb{R}^2_+$$

The *shape* of $\tau_i$ is the angle $\theta = \arctan(b/a) \in (0, \pi/2)$.

**Remark.** The previous formulation defined the triangle as an ordered triple $(\delta_1, \delta_2, \delta_3)$ with $\delta_3 = \delta_1 + \delta_2$. This third component is redundant—it carries no additional information beyond the pair. The corrected definition uses the minimal representation. Calling this a "2-simplex" was geometrically imprecise; the shape maps to the open 1-simplex $\{(x,y) : x+y=1, x>0, y>0\}$, which is topologically an open interval.
...
**Definition 7.5** (Log-Temporal Coordinates). For a temporal triangle $\tau = (a, b)$ with reference timescale $t_0 > 0$, the *log-temporal coordinates* are:
...
**Remark.** The original formulation defined a complex-valued interval ratio $r(\tau) = \frac{\delta_1}{\delta_3} + \frac{\delta_2}{\delta_3} \cdot \omega$. Since $\delta_1/\delta_3 + \delta_2/\delta_3 = 1$, this collapsed to a 1D line segment in $\mathbb{C}$, not the claimed equilateral triangle. The log-temporal coordinates $(X, Y)$ correctly preserve the 2D structure of the classification.
...
**Open gap in proof.** The snap-commutation property must be verified for the *new* temporal triangles formed in the merged stream by interleaved timestamps—not assumed. These merged triangles differ from the original triangles in $S_1$ and $S_2$. Verification that the morphisms from $T$ preserve snap-commutation through the product construction remains to be completed. This result should be treated as a plausible conjecture pending full proof.
...
**Open gap in proof.** The construction introduces an asymmetry (ordering $S_1$ before $S_2$) and an arbitrary parameter $\epsilon$. The junction triangle spanning the artificial gap is an artifact. Verification that junction triangles satisfy snap-commutation for morphisms remains to be completed.
...
**Open gap in proof.** The monad laws require verifying that temporal streams match at every point including absence markers, and that the unit and multiplication are natural transformations commuting with TStream morphisms. The statement "monad laws follow from associativity of temporal concatenation" does not engage with the actual monadic structure ($\eta$ creates spawn-return pairs, not just concatenation). This result should be treated as a plausible conjecture pending full proof. A Coq formalization is identified as future work (Problem 3, Chapter 10).
...
$$H^1_{\check{C}}(X, F) = 0 \iff \text{no temporal anomalies in } X$$
...
*Proof.* ($\Rightarrow$) If $H^1(X, F) = 0$, then every compatible family on a cover extends to a global section. This means that for any two overlapping intervals $U, V$ in $X$, the observations on $U$ and $V$ agree on $U \cap V$. This is precisely the condition for no temporal anomaly.

($\Leftarrow$) If there are no temporal anomalies, then for any cover $\{U_i\}$ of $X$, any compatible family $\{s_i\}$ has consistent overlap values. By the gluing axiom, this family extends to a global section. Thus the first Čech cohomology vanishes.

**Remark.** The converse direction requires the observation that $X$ is paracompact (as a subset of $\mathbb{R}$) and $F$ is a sheaf (not merely a presheaf), ensuring that Čech and sheaf cohomology coincide. For a full treatment, see Tennison, *Sheaf Theory* (1975), Theorem 4.4.5.
...
**Theorem 7.16** (DepCat Groupoid — Corrected). If spawn-return is modeled as typed morphism pairs (spawn: $A_j \to A_i$, return: $A_i \to A_j$), then the free groupoid generated by these pairs is well-defined if and only if all spawns have corresponding returns.

*Proof.* In the corrected formulation, each spawn-return pair $(s: A_j \to A_i, r: A_i \to A_j)$ generates an isomorphism in the groupoid completion: $r \circ s = \text{id}_{A_j}$ and $s \circ r = \text{id}_{A_i}$ when the return completes successfully. If all spawns return, every generator is invertible, and the groupoid is well-defined.

**Remark.** The original formulation conflated "dependency" (a structural DAG relationship) with "spawn-return" (a protocol pattern). The inverse of a dependency is *not* a return—it is a reverse dependency, which creates a cycle. The corrected version requires explicitly typed morphisms (spawn vs. return) rather than a single morphism type.
...
- **Associativity**: $\mu \circ T_\bot(\mu) = \mu \circ \mu(T_\bot)$. Nested absence-detection followed by flattening is independent of flattening order, since absence-of-absence = presence is associative.

**Important caveat.** These monad law verifications are informal. A rigorous proof requires showing that $T_\bot$ is an endofunctor on TStream (not merely a functor to an extended stream category), and that $\eta$ and $\mu$ are natural transformations commuting with TStream morphisms. The external parameter $\mu$ (T-0 clock interval) introduces a dependency that may violate endofunctoriality. A full Coq formalization is identified as future work (Problem 3, Chapter 10). $\square$
...
**Observation 7.22** (Binary Consensus as Lattice Snap — Downgraded from Theorem). Binary consensus protocols can be trivially represented as snap-to-lattice operations on 2-point lattices. In Raft, each log entry has a binary state (committed/uncommitted); this is equivalent to snapping temporal observations to the lattice $\mathbb{Z}$ with only two relevant states.

**Remark.** This observation is trivially true—any binary classification can be described as snapping to a 2-point lattice. It provides no insight into Raft's actual protocol (leader election, log matching, safety proof) and is included for categorical completeness only. The original formulation as a "Theorem" overstated the result's significance.

**Corollary 7.23.** Any classification that reduces temporal information to a finite set of states admits a lattice-snap representation. (Downgraded from the original claim about consensus protocols, which do not operate on temporal information.)
...
**Proposition 7.27** (requires additional assumptions). Let $\mathcal{L}_{\text{sync}} = \mathbf{D} - \mathbf{H}$ be the sync Laplacian of a fleet harmony matrix $\mathbf{H} \in [0,1]^{n \times n}$. If $\mathbf{H}$ is the adjacency matrix of a $d$-regular graph with all edge weights in $[w_{\min}, w_{\max}]$, then by Cheeger's inequality:

$$\frac{\lambda_1}{2} \leq h \leq \sqrt{2\lambda_1}$$

where $h$ is the Cheeger constant (edge expansion) and $\lambda_1$ is the Fiedler value. The minimum pairwise harmony satisfies:

$$\min_{i \neq j} H(A_i, A_j) \geq \frac{\lambda_1 \cdot \delta}{2d}$$

where $\delta$ is the minimum degree. This bound requires regularity and is not directly applicable to general (non-regular) harmony matrices.
...
| 7.22 | Binary consensus as lattice snap | Observation (downgraded from Theorem) |
| 7.23 | Finite classification as finite sublattice snap | Corollary (narrowed) |
| 7.24 | Tempo derivative | Definition |
| 7.25 | Absence integral | Definition |
| 7.26 | Sync Laplacian | Definition |
| 7.27 | Fiedler–Cheeger bound on harmony | Proposition (corrected, requires regularity) |
...
**Caveat.** 14 shapes from 21 tiles means most shapes appear exactly once. With 5 shape categories and 19 triangles, the maximum categorical diversity is 5. The "14 unique shapes" count must refer to distinct Eisenstein lattice points (sub-classifications within categories), not the 5 top-level categories. This distinction was unclear in earlier drafts. With only 18 triangles, diversity may simply reflect small-sample variance rather than genuine behavioral complexity. Claims about "shape diversity" should be tempered accordingly.
...
**Caveat.** Hurst exponent estimation requires long time series for reliability. The standard R/S method requires $n > 100$ for reliable estimates (Taqqu, Teverovsky, & Willinger, 1995). With the tile counts in our corpus (forge: 21, fleet_health: 690), the estimates for low-tile agents have wide confidence intervals. The forge's $H = 0.716$ from 21 data points has an approximate 95% CI of [0.4, 1.0]—it is essentially uninformative. Fleet_health's $H = 0.655$ from 690 points is more reliable. The claim of $H \approx 0.7$ as a "creative constant" is **unsupported** at these sample sizes; it is a preliminary observation requiring validation with $n \geq 100$ tiles per room across $\geq 30$ independent creative rooms.
...
**Caveat.** These are Pearson correlations over binned time series. With small sample sizes, individual correlation coefficients have wide confidence intervals. With 12 rooms, there are $C(12,2) = 66$ possible pairwise correlations; at the 95% significance level, approximately 3.3 spurious correlations are expected by chance alone. The 6 reported correlations barely exceed this false discovery rate. After Bonferroni correction ($\alpha/66 = 0.00076$ per test), likely none survive. The anti-coupled proofs×security result ($r = -0.772$) is the most statistically robust due to longer observation windows. All connectome findings should be treated as preliminary indicators pending replication with larger samples and proper multiple-test correction (Benjamini-Hochberg FDR control).
...
**Corrected universal principle:** The direction of the information asymmetry depends on the room's miss rate $M(R)$:
...
**Consequence:** The forge room ($M = 0.70$) has hits that are 3.4× more informative than misses ($I_{\text{hit}} \approx 1.74$ bits vs. $I_{\text{miss}} \approx 0.51$ bits). The dissertation's earlier universal claim "absence is the signal" is **incorrect** for high-miss rooms. The correct statement is: *deviations from expectation carry information proportional to their rarity.*
